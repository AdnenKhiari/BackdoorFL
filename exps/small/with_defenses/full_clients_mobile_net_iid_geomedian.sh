...
HYDRA_FULL_ERROR=1 && python "$script_path" \
  ...
  poisoned_client.cfg.norm_scaling_factor=4 \
  ...
  strategy=fedgeomedian \
  defense_strategy=no_defense \
  wandb.main_run_name="mobilenet_breast_iid_full_attack_geo_median" \
  wandb.group_name="with_defense" \
  wandb.project_name="official_last"
