...
HYDRA_FULL_ERROR=1 && python "$script_path" \
  ...
  poison_between=[[40,120]] \
  global_merger=single \
  poisoned_client=simple_patch \
  poisoned_client.cfg.norm_scaling_factor=1 \
  grad_filter.active=false \
  pgd_conf.active=false \
  global_seed=42 \
  wandb.active=true \
  wandb.main_run_name="resnet_breast_simple_patch_raw_iid" \
...
HYDRA_FULL_ERROR=1 && python "$script_path" \
  ...
  global_seed=42 \
  wandb.active=true \
  wandb.main_run_name="resnet_breast_randomized_boosted_distributed_iid" \
  wandb.group_name="attacker_only" \
  wandb.project_name="federated_official"