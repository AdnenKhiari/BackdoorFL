...
class RandomizedSimplePoisonedClient(PoisonedFlowerClient):
    def __init__(self, node_id,model_cfg,optimizer,batch_poison_num,target_poisoned,batch_size,patch_location_range: tuple, patch_size_range: tuple, patch_val: float,seed,norm_scaling_factor,pgd_conf,grad_filter,poison_between) -> None:
        data_poisoner = DataPoisoningPipeline([RandomizedSimplePatchPoisoner(patch_location_range,patch_size_range,patch_val,seed)])
        super(RandomizedSimplePoisonedClient,self).__init__(node_id,model_cfg,optimizer,data_poisoner,batch_poison_num,target_poisoned,batch_size,pgd_conf,grad_filter,poison_between)
